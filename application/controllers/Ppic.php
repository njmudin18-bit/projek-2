...
class Ppic extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($check_permission->num_rows() == 1) {
			...
			$data['nama_halaman'] 	= "Monitoring Job";
			...
			//END LOG
			$this->load->view('adminx/ppic/monitoring_job', $data, FALSE);
		} else {
			redirect('errorpage/error403');
		}
	}

	public function monitoring_job_list()
	{
		...
		$WH_table 	= "Trans_BHPDT" . $year . $month; //Trans_BHPDT202212
		...
		$sql 		= " SELECT a.scan_id AS scan_prod_id, a.barcode_no AS barcode_no_prod, 
								...
								FROM tbl_scanbarcode_job 
								WHERE loc_id = 'QC001'
								GROUP BY scan_id, barcode_no, loc_id, scan_status, scan_date, scan_by) b
		...
		foreach ($result as $key => $value) {
			$no_job 		= substr($value->barcode_no_prod, 5, 19); //$value->barcode_no_prod;
			//echo $no_bukti; exit();
			$status_qc 	= $value->scan_status_qc;
			$isi 			 	= "'" . $value->barcode_no_qc . "', '" . $value->scan_qc_id . "'";
			if ($value->scan_status_qc == NULL) {
				$status_qc = '-';
			} else {
				if ($status_qc == 'NG') {
					$status_qc = '<h5 class="pointer" title="Klik untuk cek detail" onclick="cek_details(' . $isi . ')"><span class="badge badge-danger">' . $status_qc . '</span></h5>';
				} else {
					$status_qc = '<h5><span class="badge badge-success">' . $status_qc . '</span></h5>';
				}
			}
			...
			$data[] = array(
				...
				$value->scan_status_prod,
				$value->scan_date_prod,
				$value->scan_by_prod,
				$value->barcode_no_qc == NULL ? '-' : $value->barcode_no_qc,
				$value->scan_loc_qc == NULL ? '-' : '<h5><span class="badge badge-warning text-white">' . $value->scan_loc_qc . '</span></h5>',
				$value->scan_date_qc == NULL ? '-' : $value->scan_date_qc,
			...
			if (count($result) > 0) {
				...
				foreach ($result as $key => $value) {
					$text .= '<tr>
		 								...
		 								<tr>
		 									<td colspan="5" class="text-left">PENYEBAB: ' . $value->penyebab . '</td>
		 								</tr>';
				}
			} else {
				$text .= '<tr>
	...
	//FUNGSI MONITORING JOB NEW
	public function monitoring_job_range()
	{
		//CHECK FOR ACCESS FOR EACH FUNCTION
		$user_level 			= $this->session->userdata('user_level');
		$check_permission =  $this->Rolespermissions_model->check_permissions($this->contoller_name, $this->function_name, $user_level);
		if ($check_permission->num_rows() == 1) {
			...
			log_helper($log_url, $log_type, $log_data);
			//END LOG
			$this->load->view('adminx/ppic/monitoring_job_range', $data, FALSE);
		} else {
			redirect('errorpage/error403');
		}
	}

	public function monitoring_job_range_list()
	{
		$draw 			= intval($this->input->get("draw"));
		$start 			= intval($this->input->get("start"));
		$length 		= intval($this->input->get("length"));
		...
		foreach ($result as $key => $value) {
			...
			$data[] = array(
				...
				'<a href="' . base_url() . 'ppic/monitoring_job_details/' . base64_encode($no_job) . '/' . $bulan . '/' . $tahun . '" target="_blank"><button class="btn btn-danger btn-block btn-sm">DETAILS</button></a>'
			);
		}
	...
	public function monitoring_job_range_list_OLD()
	{
		$draw 			= intval($this->input->get("draw"));
		$start 			= intval($this->input->get("start"));
		$length 		= intval($this->input->get("length"));
		...
		if ($month_1 == $month_2) {
			$tbl_trans_job = "Trans_Job" . $year_2 . $month_2; //Trans_Job202302
			$left_join 		.= "LEFT JOIN $tbl_trans_job b on b.NoBukti = a.no_job";
		} else {
			...
			for ($i = 0; $i < $interval; $i++) {
				...
				if ($i < $interval - 1) {
					$left_join 		.= " LEFT JOIN ( (SELECT NoBukti, Tgl, PartID, UnitID, QtyOrder, Keterangan, WHResult FROM Trans_Job$tempTableName) UNION ALL";
				} else {
					$left_join 		.= " (SELECT NoBukti, Tgl, PartID, UnitID, QtyOrder, Keterangan, WHResult FROM Trans_Job$tempTableName) ) b ON b.NoBukti = a.no_job";
				}
			}
		}
		...
		//GET JENIS PART
		if ($jenis_part == 'All') {
			$where = "WHERE CAST(b.Tgl AS date) BETWEEN '$start_date' AND '$end_date'";
		} elseif ($jenis_part == 'Power Cord') {
			$where = "WHERE CAST(b.Tgl AS date) BETWEEN '$start_date' AND '$end_date' 
    						AND b.WHResult IN ('WH-FG01', 'WH-GRS01')";
		} else {
			$where = "WHERE CAST(b.Tgl AS date) BETWEEN '$start_date' AND '$end_date' 
    						AND b.WHResult IN ('WH-FG', 'WH-GRS00')";
		}
	...
	public function monitoring_job_new()
	{
		//CHECK FOR ACCESS FOR EACH FUNCTION
		$user_level 			= $this->session->userdata('user_level');
		$check_permission =  $this->Rolespermissions_model->check_permissions($this->contoller_name, $this->function_name, $user_level);
		if ($check_permission->num_rows() == 1) {
			...
			log_helper($log_url, $log_type, $log_data);
			//END LOG
			$this->load->view('adminx/ppic/monitoring_job_new', $data, FALSE);
		} else {
			redirect('errorpage/error403');
		}
	}
	...
	public function monitoring_job_details($id, $bulan, $tahun)
	{
		...
		if ($check_permission->num_rows() == 1) {
			$data['group_halaman'] 	= "PPIC";
			$data['nama_halaman'] 	= "Monitoring Job Details";
			$data['icon_halaman'] 	= "icon-airplay";
			$data['perusahaan'] 		= $this->perusahaan->get_details();
			...
			log_helper($log_url, $log_type, $log_data);
			//END LOG
			$this->load->view('adminx/ppic/monitoring_job_details', $data, FALSE);
		} else {
			redirect('errorpage/error403');
		}
	}
